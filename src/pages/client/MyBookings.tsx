...
const MyBookings: React.FC = () => {
  ...
  async function fetchBookings() {
    ...
    try {
      ...
      const data = await pyFetch(`/api/bookings?user_id=${encodeURIComponent(userId)}`, { 
        method: 'GET', 
        useApiKey: false 
      });
          ...
          try {
            ...
            if (property) {
              ...
              // Fallback to owner info if no agent
                ...
                try {
                  const ownerData = await pyFetch(`/api/users/${property.owner_id}`, { 
                    method: 'GET', 
                    useApiKey: false 
                  });
                  if (ownerData) {
                  ...
                  console.log('[MyBookings] Could not fetch owner info:', err);
                }
              }
            }
