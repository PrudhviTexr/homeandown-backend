...
const SellerDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch inquiries
      const inquiriesResponse = await fetch('/api/seller/inquiries', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
        },
      });
      ...
      // Fetch bookings
      const bookingsResponse = await fetch('/api/seller/bookings', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
        },
      });
  ...
  const handleBookingStatusUpdate = async (bookingId: string, newStatus: string, newDate?: string) => {
    try {
      const response = await fetch(`/api/seller/bookings/${bookingId}/update-status`, {
        method: 'POST',
        headers: {
  ...
  const handleInquiryResponse = async (inquiryId: string, responseMessage: string) => {
    try {
      const response = await fetch(`/api/seller/inquiries/${inquiryId}/respond`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
