...
const SellerSignup: React.FC<SellerSignupProps> = ({ onToggleMode }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'id' | 'business' | 'address') => {
  ...
  const handleStep1Submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (debug) {
      console.groupCollapsed('[SellerSignup] Step 1 submit');
      const { password, confirmPassword, ...rest } = formData;
      console.debug('Form data (sanitized):', rest);
      console.groupEnd();
    }
    if (validateStep1()) {
      if (debug) console.info('[SellerSignup] Step 1 valid -> moving to step 2');
      setStep(2);
    }
  };
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.terms_accepted) {
      if (debug) console.warn('[SellerSignup] Terms not accepted');
      toast.error('Please accept the terms and conditions');
      return;
    }

    setLoading(true);
    if (debug) console.groupCollapsed('[SellerSignup] Final submit');
    ...
    try {
      ...
      if (result.error) {
        if (debug) console.error('[SellerSignup] signUp error:', result.error);
      ...
      if (debug) console.info('[SellerSignup] Success -> showing SignupSuccess');
      setShowSuccess(true);
    } catch (error) {
      console.error('Authentication error:', error);
      toast.error('An unexpected error occurred. Please try again.');
    } finally {
      if (debug) console.debug('[SellerSignup] Finalize -> setLoading(false)');
            ...
            <form onSubmit={handleStep1Submit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full p-4 border ${errors.first_name ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors`}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full p-4 border ${errors.last_name ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors`}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full p-4 border ${errors.email ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors`}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handleInputChange}
                      className={`w-full p-4 border ${errors.password ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors pr-12`}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handleInputChange}
                      className={`w-full p-4 border ${errors.confirmPassword ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors pr-12`}
            ...
            <form onSubmit={handleFinalSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Enter business name or your name as property owner"
                />
              ...
              <div className="space-y-4 mt-6">
                <h3 className="font-medium text-gray-800">Business Verification Documents</h3>
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-green-500">
                    <input
                      ...
                      accept="image/*,.pdf"
                    />
                    <label htmlFor="business-document" className="cursor-pointer">
                      <div className="text-sm text-gray-600">
                        {businessDocument ? businessDocument.name : 'Click to upload business proof (PNG, JPG, PDF)'}
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div className="bg-green-50 p-6 rounded-lg border border-green-200">
                <h4 className="font-medium text-green-900 mb-3">Seller Benefits</h4>
                <ul className="text-sm text-green-800 space-y-2">
                  <li>• List unlimited properties for free</li>
                  <li>• Access to verified buyers and tenants</li>
                  <li>• Professional property management tools</li>
                  <li>• Market insights and pricing analytics</li>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <label className="flex items-start space-x-3">
                  <input
                    ...
                    onChange={handleInputChange}
                    className="text-green-600 focus:ring-green-500 h-4 w-4 mt-1"
                    required
                  />
                  <span className="text-sm text-gray-700">
                    I agree to the <a href="#" className="text-green-600 hover:underline font-medium">Seller Terms of Service</a> and{' '}
                    <a href="#" className="text-green-600 hover:underline font-medium">Privacy Policy</a>.
                    I confirm that I have the legal right to sell/rent the properties I will list, and I understand the commission structure.
                  </span>
                </label>
              </div>
