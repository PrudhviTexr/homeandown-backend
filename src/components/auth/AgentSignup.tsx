...
const AgentSignup: React.FC<AgentSignupProps> = ({ onToggleMode }) => {
  ...
  id_proof_file: formData.id_proof_file,
      ...
      // Upload ID proof and set verification status after successful signup
      if (result.user?.id && formData.id_proof_file) {
        try {
          ...
          await pyFetch(`/api/admin/users/${result.user.id}/verify-status`, {
            method: 'POST',
          ...
          if (debug) console.info('[AgentSignup] ID proof uploaded and verification status set to pending');
        } catch (error) {
          // Non-blocking failure - log but continue with signup success
          console.error('[AgentSignup] Failed to upload ID proof or set verification status:', error);
        }
      }
  ...
  if (debug) console.info('[AgentSignup] Success -> showing SignupSuccess');
  setShowSuccess(true);
    } catch (error) {
