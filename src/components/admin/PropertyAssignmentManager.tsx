...
import { Building2, UserCog, MapPin, DollarSign, Eye, Edit, Trash2, Plus, AlertTriangle, CheckCircle, XCircle, History, RefreshCw } from 'lucide-react';
import { AdminApi } from '@/services/pyApi';
import toast from 'react-hot-toast';
import { useNavigate } from 'react-router-dom';
import PropertyApprovalModal from './PropertyApprovalModal';
import EditPropertyModal from './EditPropertyModal';
import PropertyAssignmentTrackingModal from './PropertyAssignmentTrackingModal';
...
const PropertyAssignmentManager: React.FC<PropertyAssignmentManagerProps> = ({ onRefresh }) => {
  ...
  const [trackingPropertyId, setTrackingPropertyId] = useState<string | null>(null);
  ...
  const fetchAllData = async () => {
    try {
      ...
      setAgents((usersData || []).filter((u: any) => u.user_type === 'agent'));
  ...
  const unassignAgent = async (propertyId: string) => {
    try {
      setAssigning(propertyId);
      await AdminApi.assignAgentToProperty(propertyId, '');
  ...
  const retryPropertyAssignment = async (propertyId: string) => {
    if (!window.confirm('Retry the agent notification process for this property? This will restart the sequential notification queue.')) return;
    
    try {
      setAssigning(propertyId);
      const result = await AdminApi.retryPropertyAssignment(propertyId);
      console.log('[PropertyAssignmentManager] Retry result:', result);
      toast.success('Agent notification process restarted successfully');
      fetchAllData();
      onRefresh?.();
    } catch (error) {
      console.error('Error retrying assignment:', error);
      toast.error('Failed to retry assignment');
    } finally {
      setAssigning(null);
    }
  };
  ...
  const filteredProperties = properties.filter(property => {
    const agentId = property.agent_id || property.assigned_agent_id;
    switch (filter) {
      case 'assigned':
  ...
  const getStats = () => {
    ...
    // Check both agent_id and assigned_agent_id fields for assignment
    ...
    // Handle verified field: check for explicit true (boolean, string 'true', or 1)
    const verified = properties.filter(p => {
      const verifiedValue = p.verified;
      return verifiedValue === true || verifiedValue === 'true' || verifiedValue === 1;
    }).length;
    
    // Pending should be properties that are explicitly not verified (false/null/undefined)
    ...
    console.log('[PropertyAssignmentManager] Stats calculation:', {
      total,
      assigned,
      unassigned,
      verified,
      ...
      sampleProperty: properties[0] ? {
        ...
        assigned_agent_id: properties[0].assigned_agent_id,
        verified: properties[0].verified,
  ...
  return (
    <div className="space-y-6">
              ...
              {filteredProperties.map((property) => {
                ...
                return (
                  <tr key={property.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-4">
                      {(property.agent_id || property.assigned_agent_id) ? (
                        <div>
                          <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            {getAgentName(property.agent_id || property.assigned_agent_id)}
                          </span>
                          <div className="text-xs text-gray-400 mt-1">
                            {(property.agent_id || property.assigned_agent_id).substring(0, 8)}...
                          </div>
                        </div>
                      ) : (
                    ...
                    <td className="p-4">
                      <div className="flex space-x-2">
                        ...
                        <button 
                          onClick={() => {
                            setTrackingPropertyId(property.id);
                            setIsTrackingModalOpen(true);
                          }} 
                          className="p-1 text-gray-400 hover:text-purple-600" 
                          title="View Assignment Tracking"
                        >
                        ...
                        {(property.agent_id || property.assigned_agent_id) ? (
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              title="Retry Agent Notification (Sequential Queue)"
                              onClick={() => retryPropertyAssignment(property.id)}
                              disabled={assigning === property.id}
                            >
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {agents.map((agent) => {
              const agentProperties = properties.filter(p => (p.agent_id === agent.id) || (p.assigned_agent_id === agent.id));
                ...
                <div key={agent.id} className="border rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-medium text-blue-600">{agentInquiries.length}</span>
                      </div>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-600">Total Bookings:</span>
                        <span className="font-medium text-green-600">{agentBookings.length}</span>
                      </div>
                      <div className="flex justify-between text-sm mb-1">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-purple-600">
                          {agentInquiries.length > 0 ? '< 24h' : 'N/A'}
                        </span>
                      </div>
                  </div>
                </div>
      ...
      {trackingPropertyId && (
        <PropertyAssignmentTrackingModal
          propertyId={trackingPropertyId}
          propertyTitle={properties.find(p => p.id === trackingPropertyId)?.title || 'Property'}
          isOpen={isTrackingModalOpen}
          onClose={() => {
            setIsTrackingModalOpen(false);
            setTrackingPropertyId(null);
          }}
        />
      )}
    </div>
